class Plugboard {
    ...
    update(min_t, t, backwards) {
        ...
        if ((!backwards && t >= min_t+1) || (backwards && t >= min_t+this.step_size-1)) {
            this.enigma.plot_right_minus(left, this.width, bottom, letter_box_size, i, 0, true);
        }
        
        // left -
        if ((!backwards && t >= min_t+this.step_size-1) || (backwards && t >= min_t+1)) {
            this.enigma.plot_left_minus(left, bottom, letter_box_size, j, 0, true);
        }
        ...
        let j_x = left+5;
        let c_x = i_x+(j_x-i_x)*min(1, t/(this.step_size-2));
        let c_y = i_y+(j_y-i_y)*min(1, t/(this.step_size-2));
        stroke(255, 200, 0);
        strokeWeight(2);
